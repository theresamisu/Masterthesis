...
class CrossAttentionLayer(nn.Module):
    ...
    """
    def __init__(self, num_modalities, dim, heads, dim_head, mlp_dim, dropout=0.):
        super().__init__()
        self.num_modalities = num_modalities
        
        # one msa and mlp layer for each modality
        self.crossattn = PreNormList(dim, CrossAttention(num_modalities, dim, heads=heads, dim_head=dim_head, dropout=dropout))
...
class CrossAttentionTransformer(nn.Module):
    ...
    def __init__(self, num_modalities, num_classes, dim, depth, heads, dim_head, mlp_dim, dropout=0.):
        ...
        for _ in range(depth):
            self.layers.append(CrossAttentionLayer(num_modalities, dim, heads, dim_head, mlp_dim, dropout))
    ...
    def forward(self, x):
        ...
        class_tokens = torch.stack([x_mod[:, :self.num_classes] for x_mod in x])
...
class CrossAttention(nn.Module):
    def __init__(self, num_modalities, dim, heads=8, dim_head=64, dropout=0.):
        super().__init__()
        self.num_modalities = num_modalities
        inner_dim = dim_head * heads
        project_out = not (heads == 1 and dim_head == dim)
    ...
    def forward(self, x):
        ...
        out = []
        for mod_idx, mod_x in enumerate(x):
            _,k_self,v_self = qkv_mod[mod_idx]
            attn = torch.zeros((b, self.heads, n, n)).to(mod_x.device)
            for mod_cross_idx in range(self.num_modalities):
                if mod_cross_idx == mod_idx: # no self attention
                    continue
                q_cross,_,_ = qkv_mod[mod_cross_idx]
                # compute cross attention 
                dots = einsum('b h i d, b h j d -> b h i j', q_cross, k_self) * self.scale
                attn +=  dots.softmax(dim=-1)
            # compute mean of attention
            attn /= self.num_modalities
            v_weighted = einsum('b h i j, b h j d -> b h i d', attn, v_self)
            # reshape back to nx head*dim_head
            v_weighted = rearrange(v_weighted, 'b h n d -> b n (h d)')
            # project from inner dimension (head*dim_head) back to d
            out += [self.to_out[mod_idx](v_weighted) + mod_x]
...
class TSViT(nn.Module):
    ...
    def __init__(self, model_config, img_res=80,  num_channels=[4], num_classes=10, max_seq_len=144): # model config must contain: patch_size=2, d=128, temporal_depth=6, spatial_depth=2, n_heads=4, dim_head=64, dropout=0., emb_dropout=0., pool='cls', scale_dim=4
        ...
        # temporal encoder
        self.temporal_transformer = CrossAttentionTransformer(self.num_modalities, self.num_classes, self.dim, self.temporal_depth, self.heads, self.dim_head,
...
if __name__ == "__main__":
    ...
    os.environ["CUDA_VISIBLE_DEVICES"]= gpu_mapping["7.0"]
    ...
    model_config = {'patch_size': patch_size, 'patch_size_time': 1, 'patch_time': 4,
                    'dim': 128, 'temporal_depth': 6, 'spatial_depth': 0, 'channel_depth': 4,
