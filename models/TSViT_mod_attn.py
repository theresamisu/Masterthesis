...
class ModalityAttentionLayer(nn.Module):
    ...
    def __init__(self, dim, heads, dim_head, mlp_dim, dropout=0.):
        ...
        self.crossattn = PreNormList(dim, ModalityAttention(dim, heads=heads, dim_head=dim_head, dropout=dropout))
    ...
    def forward(self, x):
        ...
        x = self.crossattn(x)
        x = [self.ff[mod_idx](x_mod) + x_mod for mod_idx, x_mod in enumerate(x)]
        return x

class ModalityAttentionTransformer(nn.Module):
    """
    contains a transformer encoder which applies Cross Attention.
    After each modality specific transformer encoder layer, the class tokens of each modality are extracted, fused classwise and again prepended to each of the modality feature token sequence 
    """
    def __init__(self, num_classes, dim, depth, heads, dim_head, mlp_dim, dropout=0.):
        ...
        for _ in range(depth):
            self.layers.append(ModalityAttentionLayer(dim, heads, dim_head, mlp_dim, dropout))
    ...
    def forward(self, x):
        ...
        #class_tokens = x[2][:, :self.num_classes] # class tokens of 
        #print("class tokens", class_tokens.shape)
        #x = [torch.cat((class_tokens, x_mod[self.num_classes:]), dim=1) for x_mod in x]
        # keep only the class tokens -> (B*h*w)xKxd
        # return feature tokens of planet modality
        return x[2]

class PreNormList(nn.Module):
    ...
    def forward(self, x, **kwargs):
        return self.fn([self.norm(mod_x) for mod_x in x], **kwargs)
...
class ModalityAttention(nn.Module):
    ...
    def forward(self, x):
        b, n, _, h = *x[0].shape, self.heads

        qkv_mod = []
        # compute q,k and v for all modalities
        for mod_idx, mod_x in enumerate(x):
            qkv = self.to_qkv[mod_idx](mod_x).chunk(3, dim=-1)
            q, k, v  = map(lambda t: rearrange(t, 'b n (h d) -> b h n d', h=h), qkv)
            qkv_mod += [(q,k,v)]
        
        out = []
        # compute self attention for S1 and S2
        for mod_idx in range(2):
            q_mod, k_mod, v_mod = qkv_mod[mod_idx]
            dots = einsum('b h i d, b h j d -> b h i j', q_mod, k_mod) * self.scale
            #print("Q^T K/ sqrt(d_head)",dots.shape)
            # attention between each of the n tokens: nxn. per head: hxnxn
            attn = dots.softmax(dim=-1)
            # weightening the token values V (hxnxd) with a (hxnxn) -> out = hxnxd 
            v_weighted = einsum('b h i j, b h j d -> b h i d', attn, v_mod)
            # reshape back to nx head*dim_head
            v_weighted = rearrange(v_weighted, 'b h n d -> b n (h d)')
            # project from inner dimension (head*dim_head) back to d
            out += [self.to_out[mod_idx](v_weighted) + x[mod_idx]]

        # use keys and values of S1 and S2 for cross attention for Planet
        q_S1,k_S2,v_Planet = qkv_mod[0][0], qkv_mod[1][1], qkv_mod[2][2]
        # compute cross attention 
        dots = einsum('b h i d, b h j d -> b h i j', q_S1, k_S2) * self.scale
        attn =  dots.softmax(dim=-1)
        # compute mean of attention
        v_weighted = einsum('b h i j, b h j d -> b h i d', attn, v_Planet)
        # reshape back to nx head*dim_head
        v_weighted = rearrange(v_weighted, 'b h n d -> b n (h d)')
        # project from inner dimension (head*dim_head) back to d
        out += [self.to_out[2](v_weighted) + x[2]]
...
class TSViT(nn.Module):
    ...
    def __init__(self, model_config, img_res=80,  num_channels=[4], num_classes=10, max_seq_len=144): # model config must contain: patch_size=2, d=128, temporal_depth=6, spatial_depth=2, n_heads=4, dim_head=64, dropout=0., emb_dropout=0., pool='cls', scale_dim=4
        ...
        # temporal encoder
        self.temporal_transformer = ModalityAttentionTransformer(self.num_classes, self.dim, self.temporal_depth, self.heads, self.dim_head,
                                                self.dim * self.scale_dim, self.dropout)
        self.planet_temporal_transformer = Transformer(self.dim, self.temporal_depth, self.heads, self.dim_head, self.dim * self.scale_dim, self.dropout)
    ...
    def forward(self, x):
        ...
        print("patch embedding\t", torch.cuda.memory_allocated(x[0].device)* 1e-06, "Mb")
        ...
        print("temporal transformer\t", torch.cuda.memory_allocated(x.device)* 1e-06, "Mb")
        ...
        x = self.planet_temporal_transformer(x)
        # keep only the class tokens -> (B*h*w)xKxd
...
if __name__ == "__main__":
    ...
    model = TSViT(img_res=res, num_channels=channels, model_config=model_config, num_classes=num_classes).to("cuda:1")
